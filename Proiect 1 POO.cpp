//linia 370 - de pe StackOverflow(https://stackoverflow.com/questions/2218290/concatenate-char-array-in-c)
//liniile 51-56 - Stack Overflow (https://stackoverflow.com/questions/997946/how-to-get-current-time-and-date-in-c)
#include <iostream>
#include <ctime>
#include <cstring>
using namespace std;
...
public:
    ...
    int getLuna() const                                           //getter pentru luna
    {return this->luna;}
    ...
    Programare():nr_programare(contor_prog++)                                        //constructor fara parametri, va lua data curenta
    {
    ...
    Programare& operator = (const Programare& prog)                         //supraincarcarea = pentru clasa Programare
    {
        if(this != &prog)
        {
            ...
            int *idPtr3 = (int*) &this->nr_programare;              //artificiu pentru a copia si nr de la un obiect la altul, salvez in int non-const pt a putea copia obiectele identic
            *idPtr3 = prog.nr_programare;
        }
        return *this;
    }
    ...
    friend ostream& operator << (ostream& out, const Programare& p)                 //supraincarcare <<
    {
        ...
        out<<"Numarul programarii este "<< p.nr_programare<<endl;
        return out;
    }
...
public:
    ...
    Doctor(char* nume, const string& prenume, const string& specializare, float rating):id_doctor(contor_doctor++)    //constructor cu toti parametri
    {
        this->nume=new char[strlen(nume)+1];
        strcpy( this->nume, nume);
        this->prenume = prenume;
    ...
    {
        Doctor doc1;
        sprintf(doc1.nume,"%s si %s", doc.nume, nume);           // preluat de pe Stack Overflow
    ...
    Doctor operator+(float x)                                   //supraincarcare + pentru float; ob + x
    {
        Doctor aux(*this);
...
public:
    ...
    Pacient& operator = (const Pacient& p)                              //supraincarcare operator = pentru Pacient
    {
        if(this != &p)
        {
            ...
            if(this->vizite_doctor_ultimul_an != nullptr)
                delete[] vizite_doctor_ultimul_an;
            ...
            int *idPtr1 = (int*) &this->cod_pacient;                    //artificiul pt copiere identica
            *idPtr1 = p.cod_pacient;
        }
        return *this;
    }
    ...
    friend ostream& operator << (ostream& out, const Pacient& p)                 //supraincarcare <<
    {
        ...
        out<<"Email-ul pacientului/tutorelui legal este "<< p.email<<endl;
        if(p.asigurare == 1) out << "Pacientul este asigurat"<<endl;
        else if(p.asigurare == 0) out << "Pacientul nu este asigurat"<<endl;
    ...
    friend istream& operator >> (istream& in, Pacient& p)                        //supraincarcare >>
    {
        ...
        in >> p.varsta;
        cout << "Email-ul pacientului/tutorelui legal este: "<<endl;
        in >> p.email;
    ...
    friend Pacient operator + (const Programare& prog, Pacient p);                                      //declarata in afara clasei
    friend Pacient operator + (Pacient p, const Programare& prog)
    {
        int idx = prog.getLuna();
        p.vizite_doctor_ultimul_an[idx-1]++;                        //pun index-1 ca sa se pastreze ordinea lunilor incepand de la 1 la 12, nu 0-11
        return p;
    }
};

Pacient operator + (const Programare& prog, Pacient p)                      //prin (prog+pacient) voi lua luna in care a fost programarea si voi incrementa cu 1 pozitia aferenta din vectorul de frecventa la clinica
{
    int idx = prog.getLuna();
    p.vizite_doctor_ultimul_an[idx-1]++;                        //pun index-1 ca sa se pastreze ordinea lunilor incepand de la 1 la 12, nu 0-11
    return p;
...
public:
    ...
    Serviciu(const string& serviciu_oferit, double tarif, float reducere):cod_serviciu(contor_serviciu++)          //constructor cu toti parametri
    {
        ...
        if (reducere == 0)
            this->tarif = tarif;
        else this->tarif = reducere * tarif;
    ...
    Serviciu& operator = (const Serviciu& serv)                                //supraincarcarea operatorului = pentru Serviciu
    {
        if(this != &serv)
        {
            ...
            int *idPtr2 = (int*) &this->cod_serviciu;
            *idPtr2 = serv.cod_serviciu;
        }
        return *this;
    }
    ...
    Serviciu operator+(Serviciu const &serv)                                       //supraincarcare + pentru Servicii. Tarifele se aduna si serviciile oferite se concateneaza
    {
        Serviciu serv1;
        serv1.tarif = serv.tarif + tarif;
        serv1.reducere = (serv.reducere + reducere)/2;
    ...
    friend ostream& operator << (ostream& out, const Serviciu& serv)                 //supraincarcare <<
    {
        ...
        out<<"Tariful serviciului este " <<serv.tarif<<endl;
        out<< "Reducerea serviciul este de " << serv.reducere<<endl;
        out<<"Codul serviciului este de "<<serv.cod_serviciu<<endl;
        return out;
    }
    ...
    friend istream& operator >> (istream& in, Serviciu& serv)                        //supraincarcare >>
    {
        ...
        if (serv.reducere == 0) serv.tarif = serv.tarif;
        else serv.tarif = serv.tarif * (1-serv.reducere);
        return in;
    }
...
void optiuniMeniu(){                                                //functie pentru afisarea optiunilor
    ...
    cout<<"Pentru a naviga printre doctori, alegeti optiunea 9 "<<endl;                                             //unde folosesc supraincarcarile de operatori pt Doctor
    cout<<"Pentru a naviga printre clientii clinicii, alegeti optiunea 10 "<<endl;                                  //unde folosesc supraincarcarile de operatori pt Pacient
    cout<<"Pentru a naviga printre programari, alegeti optiunea 11 "<<endl;                                         //unde folosesc supraincarcarile de operatori pt Programare
    cout<<"Pentru a naviga printre serviciile oferite, alegeti optiunea 12 "<<endl;                                 //unde folosesc supraincarcarile de operatori pt Serviciu
...
void optiuneaUnu(Doctor* vec[], Pacient* pac[], Programare* prog[], Serviciu* serv[], int &sizeVec,int &nrVec, int &sizePac,int &nrPac, int &sizeServ, int &nrServ, int &sizeProg, int &nrProg) {
    while (1)
    {
        ...
        if (opt == 1) {
            if(nrVec + 1 > sizeVec)                             //daca nr de doctori ajunge la limita maxima alocata de sizeVec, voi suplimenta
            {
                ...
                *vec = tmp;                             //vec devine tmp
                sizeVec += 10;
                cin>>(*vec)[nrVec++];               //aici citesc noul obiect
                break;
            }
                ...
                if(i == nrVec) {cin>>(*vec)[nrVec++]; break;}           //daca ajung la i == nrVec inseamna ca toate pozitiile sunt ocupate, deci voi introduce obiectul pe pozitia nrVec si incrementez nrVec
            }
        ...
        else if (opt == 4)                                      //aceiasi pasi ca la extinderea vectorului vec
        {
        ...
        if (opt == 1) {
            ...
            for (i = 0; i < nrVec; i++) {
                if (vec[i].getId_doctor() == id && vec[i].getPrenume() != "Anonim") {
                    cin >> vec[i];                              //modificare presupune reintroducerea obiectului cu cin, nu doar modificarea unei singure variabile
                    break;
                }
            }
            if (i == nrVec) {
            ...
            cin >> cod;
            for (i = 0; i < nrPac; i++) {
                if (pac[i].getCod_pacient() == cod && pac[i].getPrenume() != "Anonim") {
                    cin >> pac[i];                                  //modificare inseamna recitirea cu totul a obiectului respectiv
                    break;
                }
            }
            if (i == nrPac) {
            ...
            cin >> nr;
            for (i = 0; i < nrProg; i++) {
                if (prog[i].getNr_programare() == nr && prog[i].getSpecializare() != "") {
                    cin >> prog[i];
                    break;
                }
            }
...
//functia de stergere a datelor. nu voi sterge fizic un obiect, il voi lasa in vector, dar ii voi seta anumiti parametri la "Anonim" sau "", astfel incat sa indice ca obiectele sunt inactive.
//daca voi dori sa fac inserarea unui nou obiect, atunci il voi pune pe prima pozitie unde gasesc un obiect "sters"

void optiuneaTrei(Doctor vec[], Pacient pac[], Programare prog[], Serviciu serv[], int nrVec, int nrPac, int nrServ, int nrProg){
    ...
    while (1){
        if (opt == 1) {
            ...
            cin >> id;
            for (i = 0; i < nrVec; i++) {
                if (vec[i].getId_doctor() == id && vec[i].getPrenume() != "Anonim") {           //verific ca obiectul sa nu fi fost sters deja
                    cout << "Doctorul cu id-ul " << id << " a fost sters" << endl;
                    vec[i].setPrenume("Anonim");              //prin prenumele setat la "Anonim" indic ca acel doctor nu mai exista
                    break;
                }
            }
            ...
            for (i = 0; i < nrPac; i++) {
                if (pac[i].getCod_pacient() == cod && pac[i].getPrenume() != "Anonim") {
                    ...
                    pac[i].setPrenume("Anonim");          //acelasi lucru ca la doctor, setez prenumele la "Anonim" pentru a sti daca un obiect a devenit inactiv
                    break;
                }
            }
            ...
            cin >> cod;
            for (i = 0; i < nrServ; i++) {
                if (serv[i].getCod_serviciu() == cod && serv[i].getServiciu_oferit() != "") {           //verific sa nu fi fost sters deja
    ...
    while (1){
        if (opt == 1) {
            ...
            if (optiune == 1){
                for(int i = 0; i < nrVec; i++)
                {
                    if(vec[i].getPrenume() != "Anonim")                 //prin aceasta verificare doresc sa nu afisez obiectele care inca exista in vector pana la nrVec, dar care au fost sterse anterior
                    {
                ...
                cout<<"Introduceti ID-ul doctorului cautat"<<endl;          //voi cauta prin vector dupa id
                int ID;
                cin>>ID;
                int i;
                for(i=0; i < nrVec; i++)
            ...
            if (optiune == 1){
                ...
                for(int i=0; i<nrPac; i++)
                {
                    if (pac[i].getCod_pacient() == ID && pac[i].getPrenume() != "Anonim")
                    {cout<<pac[i]<<endl;
                        break;}
        ...
        else if (opt == 3)                                                          //exact aceiasi pasi ca mai sus, in vectorul de Doctori si Pacienti
        {
            cout<<"Daca doriti afisarea tuturor programarilor, alegeti optiunea 1"<<endl;
            cout<<"Pentru afisarea unei singure programari cunoscute dupa numar, alegeti optiunea 2"<<endl;
            cout<<"Pentru a reveni la meniul principal, alegeti orice alta optiune"<<endl;
            int optiune;
            cin>>optiune;
            if (optiune == 1){
        ...
        else if (opt == 4) {                                                                //exact aceiasi pasi ca mai sus, in vectorul de Doctori, Pacienti si Programari
            ...
            if (optiune == 1){
                ...
                for(int i=0; i<nrServ; i++)
                {
                    if (serv[i].getCod_serviciu() == nr && serv[i].getServiciu_oferit() != "")
                    {cout<<serv[i]<<endl;
                        break;
                    }
    ...
    while (1)
    {
        if (opt == 1) {
            ...
            for (int i = 0; i < nrProg; i++) {
                ...
                {cout << string(prog[i]) << ", la ora " << prog[i].getOra_programare()<< endl; nr++;}
            }
        ...
        else if (opt == 2)
        {
            ...
            if (nr == 1) break;
                ...
                int i;
                for (i = 0; i < nrProg; i++) {
                    if (prog[i].getNr_programare() == cod && prog[i].getSpecializare() != "")               //verificare dupa numar (plus ma asigur ca obiectul este inca activ)
                    {
...
int main()
{
    ...
    while (1)
    {
        ...
        if (nrOpt == 1) {optiuneaUnu(&vec,&pac,&prog,&serv, sizeVec, nrVec, sizePac, nrPac,sizeServ, nrServ,sizeProg,nrProg);}
        ...
        else if (nrOpt == 3) {optiuneaTrei(vec,pac,prog,serv,nrVec, nrPac,nrServ, nrProg);}
            ...
            {
                if(vec[i].getPrenume() != "Anonim" ) vec[i].filtruDoctor(specializare,rating);
            }
        }
        else if (nrOpt == 6){
            ...
            for(i = 0; i < nrPac; i++)
            {
                if (pac[i].getCod_pacient() == cod && pac[i].getPrenume() != "Anonim")          //verficare cod si prenume sa fie diferit de "Anonim"
                {
                    {pac[i].calculPachetPromotional(); break;}
                }

            }
            if (i == nrPac) cout<< "Codul nu a fost gasit in sistem"<<endl;
            ...
            for(int i = 0; i < sizeProg; i++)
                {
                    if(prog[i].getSpecializare() != "") prog[i].calculProgramari(zi,luna,an,nr);
                }
